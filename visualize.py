...
#       continue
    ...
    print(bboxes,"bboxes")
    Leye_x = np.floor(y_pred[0]*x_product+min_x)
    Leye_y = np.floor((96-y_pred[1])*y_product+min_y)
...
def nosefilter(img,Leye_x,Leye_y,nose_x,nose_y,mouse_x,mouse_y):
    ...
    while scale>0.5:
        ...
        radius_x = 3*scale*abs(float(Leye_x)-center_x)
        radius_y = 3*scale*abs(float(Leye_y)-center_y)
        ...
        for y in range(int(min_b),int(max_b)):
            for x in range(int(min_a),int(max_a+max_b*cosine_sita)):
